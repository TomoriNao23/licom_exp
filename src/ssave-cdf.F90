...
!
#ifdef SPMD
      call local_to_global(hist_output,rest_output)
         ...
         DO j = 1,jmt_global
            DO i = 1,imt
               ...
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= z0mon_io (i,j)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= z0mon (i,j)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= himon_io (i,j)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= himon (i,j)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= hdmon_io (i,j)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= hdmon (i,j)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= icmon_io (i,j,1)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= icmon (i,j,1)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= icmon_io (i,j,2)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= icmon (i,j,2)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= netmon_io (i,j,1)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= netmon (i,j,1)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= netmon_io (i,j,2)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= netmon (i,j,2)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= mldmon_io (i,j)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= mldmon (i,j)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
         ...
         DO j = 1,jmt_global
            DO i = 1,imt
               ...
               IF (viv_global (i,j,k) > 0.5) THEN
                  t3_cdf (i,j,k,1)= akmmon_io (i,j,k)/ NSS/(nmonth (mon0))
#else
               IF (viv (i,j,k) > 0.5) THEN
                  t3_cdf (i,j,k,1)= akmmon (i,j,k)/ NSS/(nmonth (mon0))
#endif
               ELSE
         ...
         DO j = 1,jmt_global
            DO i = 1,imt
               ...
               IF (vit_global (i,j,k) > 0.5) THEN
                  t3_cdf (i,j,k,1)= aktmon_io (i,j,k)/ NSS/(nmonth (mon0))
#else    
               IF (vit (i,j,k) > 0.5) THEN
                  t3_cdf (i,j,k,1)= aktmon (i,j,k)/ NSS/(nmonth (mon0))
#endif 
               ELSE
                  t3_cdf (i,j,k,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,k) > 0.5) THEN
                  t3_cdf (i,j,k,1)= aksmon_io (i,j,k)/ NSS/(nmonth (mon0))
#else    
               IF (vit (i,j,k) > 0.5) THEN
                  t3_cdf (i,j,k,1)= aksmon (i,j,k)/ NSS/(nmonth (mon0))
#endif 
               ELSE
                  t3_cdf (i,j,k,1)= spval
               END IF
...
#ifdef SPMD
                  IF (vit_global (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= tsmon_io (i,j,k)/ NSS/(nmonth (mon0))
#else
                  IF (vit (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= tsmon (i,j,k)/ NSS/(nmonth (mon0))
#endif
                  ELSE
                     t3_cdf (i,j,k,1)= spval
                  END IF
...
#ifdef SPMD
                  IF (vit_global (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= ssmon_io (i,j,k)*1000./NSS/(nmonth (mon0))+35.
#else
                  IF (vit (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= ssmon (i,j,k)*1000./ NSS/(nmonth (mon0)) +35.
#endif
                  ELSE
                     t3_cdf (i,j,k,1)= spval
                  END IF
...
#ifdef SPMD
                  IF (vit_global (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= wsmon_io (i,j,k)/ NSS/(nmonth (mon0))
#else
                  IF (vit (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= wsmon (i,j,k)/ NSS/(nmonth (mon0))
#endif
                  ELSE
                     t3_cdf (i,j,k,1)= spval
                  END IF
...
#ifdef SPMD
                  IF (viv_global (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= usmon_io (i,j,k)/ NSS/(nmonth (mon0))
#else
                  IF (viv (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= usmon (i,j,k)/ NSS/(nmonth (mon0))
#endif
                  ELSE
                     t3_cdf (i,j,k,1)= spval
                  END IF
...
#ifdef SPMD
                  IF (viv_global (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= - vsmon_io (i,j,k)/ NSS/(nmonth (mon0))
#else
                  IF (viv (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= - vsmon (i,j,k)/ NSS/(nmonth (mon0))
#endif
                  ELSE
                     t3_cdf (i,j,k,1)= spval
                  END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= sumon_io (i,j)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= sumon (i,j)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= svmon_io (i,j)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= svmon (i,j)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= lthfmon_io (i,j)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= lthfmon (i,j)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= sshfmon_io (i,j)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= sshfmon (i,j)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= lwvmon_io (i,j)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= lwvmon (i,j)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
#ifdef SPMD
               IF (vit_global (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= swvmon_io (i,j)/ NSS/(nmonth (mon0))
#else
               IF (vit (i,j,1) > 0.5) THEN
                  t2_cdf (i,j,1)= swvmon (i,j)/ NSS/(nmonth (mon0))
#endif
               ELSE
                  t2_cdf (i,j,1)= spval
               END IF
...
!
       if (diag_msf) then
          do k=1,km+1
          do j=1,jmt_global
             if (psi(j,k,1)<9999999.) then
!             if (psi(j,k,1)<10000.) then
                t2z_cdf(j,k,1)=psi(j,k,1)/NSS/(nmonth (mon0))
             else
                t2z_cdf(j,k,1)=spval
             end if
          end do
...
#endif
                t2_cdf(i,j,1)=bsf(i,j)/NSS/(nmonth (mon0))
             else
                t2_cdf(i,j,1)=spval
             end if
          end do
...
#ifdef SPMD
                  IF (vit_global (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= am3mon_io (i,j,k)/ NSS/(nmonth (mon0))
#else
                  IF (vit (i,j,k) > 0.5) THEN
                     t3_cdf (i,j,k,1)= am3mon (i,j,k)/ NSS/(nmonth (mon0))
#endif
                  ELSE
                     t3_cdf (i,j,k,1)= spval
                  END IF
...
#ifdef SPMD
                     IF (vit_global (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= tsmon_io (i,j,k)/NSS/365.D0
#else
                     IF (vit (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= tsmon (i,j,k)/NSS/365.D0
#endif
                     ELSE
                        t3_cdf (i,j,k,1)= spval
                     END IF
...
#ifdef SPMD
                     IF (vit_global (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= ssmon_io (i,j,k)*1000.D0/NSS/365.D0 +35.D0
#else
                     IF (vit (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= ssmon (i,j,k)*1000.D0/NSS/365.D0 +35.D0
#endif
                     ELSE
                        t3_cdf (i,j,k,1)= spval
                     END IF
...
#ifdef SPMD
                     IF (vit_global (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= wsmon_io (i,j,k)/NSS/365.D0
#else
                     IF (vit (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= wsmon (i,j,k)/NSS/365.D0
#endif
                     ELSE
                        t3_cdf (i,j,k,1)= spval
                     END IF
...
#ifdef SPMD
                     IF (viv_global (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= usmon_io (i,j,k)/NSS/365.D0
#else
                     IF (viv (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= usmon (i,j,k)/NSS/365.D0
#endif
                     ELSE
                        t3_cdf (i,j,k,1)= spval
                     END IF
...
#ifdef SPMD
                     IF (viv_global (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= - vsmon_io (i,j,k)/NSS/365.D0
#else
                     IF (viv (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= - vsmon (i,j,k)/NSS/365.D0
#endif
                     ELSE
                        t3_cdf (i,j,k,1)= spval
                     END IF
...
#ifdef SPMD
                     IF (vit_global (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= am3mon_io (i,j,k)/NSS/365D0
#else
                     IF (vit (i,j,k) > 0.5) THEN
                        t3_cdf (i,j,k,1)= am3mon (i,j,k)/NSS/365D0
#endif
                     ELSE
                        t3_cdf (i,j,k,1)= spval
                     END IF
