...
!     dimension lenth
      integer, parameter :: lon_len=imt,lat_len=jmt_global,lev_len=km,lev1_len=km+1,time_len=1
...
!     variable shapes
      integer::  lat_dims(lat_rank),lon_dims(lon_rank),lev_dims(lev_rank),time_dims(time_rank), &
              ...
              mth_dims(mth_rank),mld_dims(mld_rank),akm_dims(akm_rank),akt_dims(akt_rank),aks_dims(aks_rank)&
             ,su_dims(su_rank),sv_dims(sv_rank)&
             ,sshf_dims(sshf_rank),lthf_dims(lthf_rank),lwv_dims(lwv_rank),swv_dims(swv_rank)

!     variables
      real(r8):: t0_cdf
      real(r4):: t2_cdf(lon_len,lat_len,time_len) ,t3_cdf(lon_len,lat_len,lev_len,time_len) 
      real(r4):: t2z_cdf(lat_len,lev1_len,time_len) 
      real(r4):: t1_cdf(lon_len,lat_len,time_len)
