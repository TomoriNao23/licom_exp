...
!Xiao Chan (Hereinafter XC for short)
      ...
      real(r8),dimension(imt,jmt,km) :: adv_xy1,adv_xy2,adv_xy3,adv_xy4, &
                adv_x0,adv_y0,adv_c1,adv_c2
      real(r8),dimension(imt,jmt,km) :: atmax,atmin,at0
      real(r8),dimension(imt,jmt,km) :: uaa,vaa,adv_yy,adv_xx,adv_zz,atz
      real(r8),dimension(imt,jmt,km)::adv_za,adv_zb1,adv_zb2,adv_zc,atmaxz,atminz
#else
...
!---------------------------------------------------------------------
      ...
      do i=1,imt
!
        if (at(i,j,1,1) < 2.0 ) then
           akt(i,j,k,1) = max ( akt(i,j,k,1), ahv_back(i,j))
           akt(i,j,k,2) = max ( akt(i,j,k,2), ahv_back(i,j))
        end if
!
      end do
      end do
...
!         DO J = JST,JET
!            DO I = 1,IMT
!               UTT (I,J)= (AA * UTF (I,J,1) + (1.0- AA)* UTL (I,J,1))*VIV(I,J,1)
!               VTT (I,J)= (AA * VTF (I,J,1) + (1.0- AA)* VTL (I,J,1))*VIV(I,J,1)
!            END DO
!         END DO
...
#ifdef SPMD
       call exchange_3d(uaa(1,1,1),km)
       call exchange_3d(vaa(1,1,1),km)
#endif
#endif
!XC
...
#ifdef SPMD
       call exchange_3d(atmax,km)
       call exchange_3d(atmin,km)
#endif
...
#ifdef SPMD
       call exchange_3d(ax(1,1,1,n),km)
       call exchange_3d(ay(1,1,1,n),km)
#endif
...
!      do j=jst,jet
!      do i=1,imt
!         tf(i,j,k)=tf(i,j,k)-trave
!      end do
!      end do
!      end do

#ifdef SPMD
       call exchange_3d(az(1,1,1,n),km)
#endif
...
!
         ...
         END DO
#ifdef SPMD
       call exchange_3d(dx(1,1,1,n),km)
       call exchange_3d(dy(1,1,1,n),km)
#endif
...
!$OMP PARALLEL DO PRIVATE (K,J,WKI)
         DO K = 1,KM
            DO J = JSM,JEM
               ...
               DO I = 2,IMM
                  WKD (I,J,K) = AH3 (I,J,K)* SOTX (J)* (WKI (I +1) - WKI (I))
               END DO
 
               WKD (1,J,K)= WKD (IMM,J,K)
               WKD (IMT,J,K)= WKD (2,J,K)
            END DO
         END DO
...
!$OMP PARALLEL DO PRIVATE (K,J,WKI)
         DO K = 1,KM
            DO J = JSM,JEM
               WKI (1)= 0.0
               DO I = 2,IMT
                  WKI (I) = (WKD (I,J,K) - WKD (I -1,J,K))* VIT (I,J,K)*VIT (I -1,J,K)
               END DO
               DO I = 2,IMM
                  TF (I,J,K) = TF (I,J,K) + SOTX (J)* (WKI (I +1) - WKI (I))
!
...
!$OMP PARALLEL DO PRIVATE (K,J,I,wt1,wt2)
         ...
         DO I = 2,IMM
            ...
            wkd (I,J,K) = ah3(i,j,k)*(R2D (J)* wt2 - R2C(J)* wt1)
         END DO
         END DO
         END DO

            DO J = JSM,JEM
               WKD (1,J,K)= WKD (JMM,J,K)
               WKD (JMT,J,K)= WKD (2,J,K)
            END DO
!         END DO
#ifdef SPMD
         call exchange_3d(wkd,km)
#endif 
!$OMP PARALLEL DO PRIVATE (K,J,I,wt1,wt2)
         DO K = 1,KM
         DO J = JSM,JEM
         DO I = 2,IMM
            wt1= (WKD (I,J,K) - WKD (I,J -1,K))* VIT (I,J,K)* VIT (I,J -1,K)
            wt2= (WKD (I,J+1,K) - WKD (I,J ,K))* VIT (I,J,K)* VIT (I,J +1,K)
            TF (I,J,K) = TF (I,J,K) + (R2D (J)* wt2 - R2C(J)* wt1)
!
            dy(i,j,k,n)=(R2D (J)* wt2 - R2C(J)* wt1)
...
#ifdef SPMD
       call exchange_3d(dy(1,1,1,n),km)
       call exchange_3d(ddy(1,1,1,n),km)
#endif
...
#endif
#ifdef SPMD
       call exchange_3d(dz(1,1,1,n),km)
#endif
...
!
            ...
            END DO
             ...
             END DO
#ifdef SPMD
         call exchange_2d(NET(1,1,2)) 
#endif
         ELSE
!$OMP PARALLEL DO PRIVATE (J,I,FAW,FIW,ALF)
            DO J = JSM,JEM
               DO I = 2,IMM
                IF (ITNU (I,J) > 0)THEN
#ifdef COUP
...
!                 STF (I,J) = (SWV(I,J)+NSWV(I,J)-SEAICE(I,J)*GAMMA*(SST (I,J) - ATB (I,J,1,1)))*OD0CP
...
#ifdef SPMD
         do j=1,jmt
            if (j_global(j)>=(jst_global+1).and.j_global(j)<=(jst_global+21)) then
#else
            DO J = JSM,JSM +20
#endif
               DO I = 2,IMM
...
#ifdef SPMD
         do j=1,jmt
            if (j_global(j)<=(jmt_global-1).and.j_global(j)>=(jmt_global-21)) then
#else
            DO J = JEM -20,JEM
#endif
               DO I = 2,IMM
                  TF (I,J,K)= TF (I,J,K) + VIT (I,J,K)* (RESTORE (I,J,K,&
                             N) - ATB (I,J,K,N))*LAMDA*2D0
               END DO
#ifdef SPMD
             endif
#endif
...
#ifdef SPMD
         call exchange_3d(trend(1,1,1,N),km) 
#endif
