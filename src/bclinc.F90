...
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1,KM
         DO J = JSM,JEM
            DO I = 2,IMM
               DLV (I,J,K) = DLV (I,J,K)  &
               -OUY (J)* (WKA (I,J +1,K) - WKA (I,J,K) + WKA (I -1,J +1,K) - WKA (I -1,J,K)) 
               DLU (I,J,K) = DLU (I,J,K)  &
               -OUX (J)* (WKA (I,J,K) - WKA (I -1,J,K) + WKA (I,  J +1,K) - WKA (I -1,J +1,K))
            END DO
         END DO
      END DO
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1,KM
         DO J = JST,JET
            DO I = 1,IMT
!lhl200100801
               WKA (I,J,K)= (1.0+ OHBT (I,J)* ZKT (K))* WORK (I,J) * VIT (I,J,K)
!????? "-" or "+"
!            WKA(I,J,K)=(1.0+OHBT(I,J)*ZKT (K))*(WORK(I,J)-elevat(i,j))*VIT(I,J,K)
            END DO
         END DO
      END DO
