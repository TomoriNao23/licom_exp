...
use forc_mod, only:su,sv,USTAR,BUOYTUR, BUOYSOL,tbv !3
#else
use forc_mod, only: su,sv,USTAR,BUOYTUR, BUOYSOL 
#endif
!linpf
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1,KM
         ...
         END DO
!lhl0710            DO I = 1,IMT
!lhl0710              wp12(i,jst,k)=wp12(i,jsm,k)
!lhl0710              wp13(i,jst,k)=wp13(i,jsm,k)
!lhl0710            END DO
      END DO
!lhl0711#ifdef SPMD 
...
!
!lhl241204
! calculate the vertical mixing on T-grid
! note the unit and the type of the variables!!!
      DO J = JSM,JEM
         DO I = 2,IMM
...
!output in cm^2/s, so 1d-4 should be multipled
               AKM_BACK(I,J,1),AKT_BACK(I,J,1),AKS_BACK(I,J,1),&
...
!input int
         ...
         DO K = 1,KM
!         AKMT(I,J,K)=+WK1(K)*1.d-4+AMV
!         AKT(I,J,K,1)=AKT(I,J,K,1)+WK2(K)/NCC*1.d-4+AHV
!         AKT(I,J,K,2)=AKT(I,J,K,2)+WK3(K)/NCC*1.d-4+AHV
!lhl201008         AKMT(I,J,K)=+WK1(K)*1.d-4
!lhl201008         AKT(I,J,K,1)=AKT(I,J,K,1)+WK2(K)/NCC*1.d-4
!lhl201008         AKT(I,J,K,2)=AKT(I,J,K,2)+WK3(K)/NCC*1.d-4
         AKMT(I,J,K)=+(WK1(K)+AKM_BACK(I,J,K))*1.d-4
         AKT(I,J,K,1)=AKT(I,J,K,1)+(WK2(K)+AKT_BACK(I,J,K))/NCC*1.d-4
         AKT(I,J,K,2)=AKT(I,J,K,2)+(WK3(K)+AKS_BACK(I,J,K))/NCC*1.d-4
         END DO
        endif
         END DO
              amld(1,j)=amld(imm,j)
...
#ifdef SPMD
      call exchange_3d(amld,1)
      call exchange_3d(akmt,km)
      call exchange_3d(akt(1,1,1,1),km)
      call exchange_3d(akt(1,1,1,2),km)
#endif
      
!linpf
#if (defined WAVEFORCE)
!         akmt = akmt + tbv ! * bv_vis_factor
         akt(:,:,:,1) = akt(:,:,:,1) + tbv ! * bv_dif_factor
         akt(:,:,:,2) = akt(:,:,:,2) + tbv !* bv_dif_factor
#endif
!linpf
...
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1,KMM1
         ...
         END DO
!            DO I = 1,IMT
!               AKMU (I,JET,K)= AKMU (I,JEM,K)
!            END DO
      END DO
#ifdef SPMD
      call exchange_3d(akmu,km)
#endif
!lhl241204
#endif
!---------------------------------------------------------------------
...
!
#ifdef CANUTO
           DLVB(I,J) = DLVB(I,J) + (diff_v1-diff_v2)*OHBU(I,J)*(1.0-AIDIF)
           DLUB(I,J) = DLUB(I,J) + (diff_u1-diff_u2)*OHBU(I,J)*(1.0-AIDIF)
#else
           DLVB(I,J) = DLVB(I,J) + (diff_v1-diff_v2)*OHBU(I,J)
           DLUB(I,J) = DLUB(I,J) + (diff_u1-diff_u2)*OHBU(I,J)
#endif
      END DO
      END DO
!
!$OMP PARALLEL DO PRIVATE (k,J,I,diff_u1,diff_v1,diff_u2,diff_v2,rkv,riv1)
      DO K = 1,KM
      DO J = JSM,JEM
         DO I = 2,IMM
!lhl1204
...
!            DO I = 2,IMM
!               DLV (I,J,K)= DLV (I,J,K) +  &
!               (R1D (J)* (amy(i,j+1,k)*VP (I,J +1,K) - amy(i,j,k)*VP (I,J,K)) &
!               - R1C (J)* (amy(i,j,k)*VP (I,J,K) - amy(i,j-1,k)*VP (I,J -1,K)) &
!               + SOUX (J)* (amy(i+1,j,k)*VP (I +1,J,K) -2.0D0* amy(i,j,k)*VP (I,J,K) + amy(i-1,j,k)*VP (I -1,J,K)) &
!               + CV1 (J)* amy(i,j,k)*VP (I,J,K) &
!               - CV2 (J)* (amy(i+1,j,k)*UP (I +1,J,K) - amy(i-1,j,k)*UP (I -1,J,K)))     
!               DLU (I,J,K)= DLU (I,J,K) +  &
!               (R1D (J)* (amx(i,j+1,k)*UP (I,J +1,K) - amx(i,j,k)*UP (I,J,K)) &
!               - R1C (J)* (amx(i,j,k)*UP (I,J,K) - amx(i,j-1,k)*UP (I,J -1,K)) &
!               + SOUX (J)* (amx(i+1,j,k)*UP (I +1,J,K) -2.0D0* amx(i,j,k)*UP (I,J,K) + amx(i-1,j,k)*UP (I -1,J,K)) &
!               + CV1 (J)* amx(i,j,k)*UP (I,J,K) &
!               + CV2 (J)* (amx(i+1,j,k)*VP (I +1,J,K) - amx(i-1,j,k)*VP (I -1,J,K)))    
!            END DO
!         END DO
!       END DO
!!!!!!!
...
!YU
#ifdef SPMD
      call exchange_3d(tmp1,km)
      call exchange_3d(tmp2,km)
#endif 
!YU
!$OMP PARALLEL DO PRIVATE (K,J,I)
      DO K = 1,KM
