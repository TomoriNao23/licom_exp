...
use shr_sys_mod
      ...
      real (r8) :: ek0,ek0li,ea0,eb0,et0,es0
      REAL (r8) :: EK,EA,EB,ET,ES
      real(r8)::t0,t1,clock0f,nst
      real,dimension(imt,jmt_global,km)::ug,vg
      save t0
      integer :: nnn
...
!!$OMP PARALLEL DO PRIVATE (K,J,I),reduction(+:EK)
      ...
      ek0= ek0*0.5D0* d0
      call local_to_global_4d_r8(u,ug,km,1)
      call local_to_global_4d_r8(u,ug,km,1)
      if(mytid==0) then
      ek0li=0.0
      DO K = 1,KM
         DO J = 1,JMT_global
            DO I = 2,IMM
               EK0li = EK0li + DZP (K)* DXDYU_global (J)* VIV_global (I,J,K) &
                    * (Ug (I,J,K)* Ug (I,J,K) + Vg (I,J,K)* Vg (I,J,K))
            END DO
         END DO
      END DO
      ek0li= ek0li*0.5D0* d0
      endif
#else
      EK0= EK *0.5D0* D0
#endif
...
!---------------------------------------------------------------------
      ...
      if (mytid==0 )then
!      WRITE (6,FMT='(I5,I3,7D25.15)') MONTH,IDAY,EK,EA,EB,ET,ES,t1-t0
      WRITE (6,FMT='(I5,I3,7D25.15)') MONTH,IDAY,EK0,EA0,EB0,ET0,ES0,t1-t0
!      WRITE (6,FMT='(I5,I3,7D25.15)') MONTH,IDAY,EK0,ek0li,EA0,EB0,ET0,ES0,t1-t0
      end if
...
use msg_mod, only: tag_1d,tag_2d,tag_3d,tag_4d,nproc,status,mpi_comm_ocn
      ...
      END SUBROUTINE chk_var2d
       ...
       SUBROUTINE chk_var(var,kk,trave)
!     =======================
...
#include <def-undef.h>
 ...
 use msg_mod, only: tag_1d,tag_2d,tag_3d,tag_4d,nproc,status,mpi_comm_ocn
       ...
       integer       :: kk
       real (kind=8) :: ek0,ea0,eb0,et0,es0
       REAL (kind=8) :: EK,EA,EB,ET,ES
       real(kind=8)::t0,t1,clock0f
       real  :: var(imt,jmt,kk)
       save t0
       integer :: nnn
       character :: ch*2
...
!!$OMP PARALLEL DO PRIVATE (K,J,I),reduction(+:EK)
 !        DO J = jsm,jem
  !         DO I = 2,imm
   !          EK = EK + h0(i,j)* DXDYT (J)* VIT (I,J,1)* VAR(I,J,1)/VSEA
    !       END DO
    !      END DO
      DO K = 1,KK
         DO J = jsm,jem
           DO I = 2,imm
             EK = EK + DZP (K)* DXDYT (J)* VIT (I,J,K)* VAR(I,J,K)/VSEA
           END DO
          END DO
      END DO

#ifdef SPMD
       call mpi_reduce(ek,ek0,1,mpi_double_precision,mpi_sum,0,mpi_comm_ocn,ierr)
       call mpi_bcast(ek0,1,mpi_double_precision,0,mpi_comm_ocn,ierr)
#else
       EK0= EK
#endif
      trave=ek0
      RETURN
      END SUBROUTINE chk_var
