...
!990928 ***Replace mikesal2 with mikesal2a to include exponential regularization option.***
	...
	back_l_0 = (((3.D0*ako)**(3.D0/2.D0))/(b1*pi_const))*back_del_0
...
!	\theta_r_Crit must be in the range (-pi/4,3pi/4) (The range of Ri>0.)
        IF(theta_rcrn.LT.-pi_const/4.D0) theta_rcrn = theta_rcrn + pi_const
        IF(theta_rcrp.LT.-pi_const/4.D0) theta_rcrp = theta_rcrp + pi_const
	theta_rcrn_deg = theta_rcrn*(180.D0/pi_const)
	theta_rcrp_deg = theta_rcrp*(180.D0/pi_const)
!       if (mytid.eq.0) then
!	WRITE(*,*) "   "
!	WRITE(*,*) "   "
!	WRITE(*,*) "   "
...
!	Increments in radial and angular coordinates in (Ri_T,Ri_C) plane.
	...
	deltheta_r = (pi_const/4.D0)/DFLOAT(n_theta_r_oct)
...
!       endif
	DO itheta_r = -n_theta_r_oct,3*n_theta_r_oct
	   ...
	   theta_r_deg = theta_r*(180.D0/pi_const)
