...
!      irec=(iyfm-1)*365+mon_day+iday-365*(500-1)
! restart from 500 year, forcing from 1986-01-01
!      irec=(iyfm-1)*365+mon_day+iday-365*(500-1)+365*(1986-1948+1) !lhl20030925
!climatology forcing
      irec=mon_day+iday
      if (mytid.eq.0) then
!      write(*,*) "iyfm=",iyfm
...
!	print*,rain3_io(180,:,1)
!	print*,snow3_io(180,:,1)
!	stop
!! modify tsa
!      do j=1,jmt_global
!! south to 60s
!         do i=178,imt
!        tsa3_io(i,j,1)=max(tsa3_io(i,j,1),-20.)
!         end do
!      end do
...
111    continue
         ...
         do j = jsm,jem
            do i = 2,imm
!              uu(i,j)=vit(i,j,1)*(utt(i,j)+utt(i+1,j)+utt(i,j-1)+utt(i+1,j-1)) &
!              /(viv(i,j,1)+viv(i+1,j,1)+viv(i,j-1,1)+viv(i+1,j-1,1)+epsln)
!              vv(i,j)=vit(i,j,1)*(vtt(i,j)+vtt(i+1,j)+vtt(i,j-1)+vtt(i+1,j-1)) &
!              /(viv(i,j,1)+viv(i+1,j,1)+viv(i,j-1,1)+viv(i+1,j-1,1)+epsln)
...
!         wspdu3(i,j,1)=(wspdu3(i,j,1)-uu(i,j))*vit(i,j,1)
!         wspdv3(i,j,1)=(wspdv3(i,j,1)-vv(i,j))*vit(i,j,1)
         windx(i,j)=(wspdu3(i,j,1)-uu(i,j))*vit(i,j,1)
         windy(i,j)=(wspdv3(i,j,1)-vv(i,j))*vit(i,j,1)
!         wspd3(i,j,1)=sqrt(wspdu3(i,j,1)**2+wspdv3(i,j,1)**2)
!         wspd3(i,j,1)=max(sqrt(wspdu3(i,j,1)**2+wspdv3(i,j,1)**2),1.0)
!  1.0 is from mom4
...
! using a transient temperature, not daily mean
         ...
         zz(i,j)=10.
!         es(i,j)=6.11*10**(7.5*(model_sst(i,j)-tok)/(237.0+model_sst(i,j)-tok))
!         qs(i,j)=0.98*0.622*es(i,j)/(psa3(i,j)-0.378*es(i,j))
         qs(i,j)=0.98*640380*exp(-5107.4/model_sst(i,j))/1.22*vit(i,j,1)
! temperature to potential temperature
         theta(i,j)=tsa3(i,j,1)*(100000.0/psa3(i,j,1))**0.286
!         else
!         wspd3(i,j,1)=0.0
!         model_sst(i,j)=0.0
!         zz(i,j)=0.0
...
!           core_sensible(1,1),core_latent(1,1),core_tau,cd(1,1), ch(1,1), ce(1,1), ustar(1,1), bstar(1,1))
...
! v=-v
!            tmp1(i,j)=core_tau(i,j)*wspdu3(i,j,1)*vit(i,j,1)
!            tmp2(i,j)=-core_tau(i,j)*wspdv3(i,j,1)*vit(i,j,1)
...
!       do j=1,jmt
!          do i=1,imt
!             swv(i,j)=swv(i,j)*(1.0d0-seaice(i,j))
!             nswv(i,j)=nswv(i,j)*(1.0d0-seaice(i,j))
!             fresh(i,j)=fresh(i,j)*(1.0d0-seaice(i,j))
!!          end do
!      end do
!! ocean-air or ocean-ice
!       do j=1,jmt
!          do i=1,imt
!             if(model_sst(i,j) <= tok-1.8) then
!! if ocean-ice, using restoring
!             swv(i,j)=0.0
!             nswv(i,j)=0.0
!             else
!! if ocean-air, using Large's bulk formulae
!             dqdt(i,j)=0.0
!             end if
!          end do
!      end do
...
!---------------------------------------------
use precision_mod
      use param_mod, only: imt,jmt,jmt_global,s_imt,s_jmt
      implicit none
#include <netcdf.inc>
...
!---------------------------------------------
use precision_mod
      use param_mod, only: imt,jmt,jmt_global,s_imt,s_jmt,mytid
...
use precision_mod
      use param_mod, only: imt,jmt,jmt_global,s_imt,s_jmt
      use pconst_mod, only: lon,lat,vit_global
      use output_mod, only: spval
      implicit none
!
      integer mx,my,ic,jc,ip,jp,i,j
      integer ind(imt,jmt_global)
      real(r8) :: a(mx,my),alat(my),alon(mx)
      real(r8) b(imt,jmt_global)
