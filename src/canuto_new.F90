...
!******Z
!030717Z1a Switch to determine whether (-1) to not to use Deardorff lengthscale, 
!	(0) to use it with a modification of {\tau N} inconsistently *after* 
!	S_{M,H,S} have been calculated (what I've done all along inherited from Ye Cheng),
...
!Z1	  because epson2_ can vary with latitude and stratification when ifdeeplat>0.
      REAL(r8) epson2,epson2_,epson2__,epson2_bot
...
!030324-27 Add shearless table as a function of the one variable (N_d^2/N^2) .
       REAL(r8) and2on2a1(-mt:mt),amtaun2a1(-mt:mt),dand2on2 &
        ,sma1(-mt:mt),sha1(-mt:mt),ssa1(-mt:mt)&
!
!030328 Ratios needed for the fast interpolation of the exponential absolute value tables.
      ,rri,rnd2on2
!******Y
...
!020923(First Day of Autumn)X Introduce an array for [\epsilon y], that is, dissipation times {\tau Shear}^2 .
      DIMENSION epsy(na)
!020924X Introduce logical variable lifupper for region where l_MLD^2 S dimensionalization makes sense
...
!       if (mytid.eq.0) then
!       print*,'in'
!       write(*,'(10d15.5)')(t(i),i=1,na)
!       write(*,'(10d15.5)')(s(i),i=1,na)
!       write(*,'(10d15.5)')(rh(i),i=1,na)
!       write(*,'(10d15.5)')(ri(i),i=1,na)
!       write(*,'(10d15.5)')(rid(i),i=1,na)
!       write(*,'(10d15.5)')(s2(i),i=1,na)
!       write(*,'(10d15.5)')(an2(i),i=1,na)
!       write(*,'(10d15.5)')(z(i),i=1,na)
!       endif
!
...
!lhl        "irimax(i)  "
!lhl	DO i= -mt,mt
!lhl           WRITE(*,9050) i,rib(i),ridb(i),irimax(i)
!lhl	END DO
!*****C
!lhl	WRITE(*,*) " "
!lhl	WRITE(*,*) "irid       Ri_d        Ri(irimax)  " &
!lhl	        // "S_M        S_H        S_S        " &
   !lhl       // "S_M/S_H    S_S/S_H    "
!lhl	DO irid= -mt,mt
!lhl	   WRITE(*,9100) irid,ridb(irid),rib(irimax(irid)), &
!lhl           smb(irimax(irid),irid), &
!lhl           shb(irimax(irid),irid), &
!lhl           ssb(irimax(irid),irid), &
!lhl    	 smb(irimax(irid),irid)/shb(irimax(irid),irid), &
!lhl           ssb(irimax(irid),irid)/shb(irimax(irid),irid)
!lhl	END DO
        endif
...
!	a subset of theta_r = -pi/4 to 3pi/4.
       if (mytid.eq.0) then
!lhl	WRITE(*,*) "S_S/S_H at pre-maximum Ri as a function of" &
!lhl             // "\\theta_r \\equiv Arctan(Ri_C/Ri_T)" 
!981220    Absurd default on sisamax \equiv S_S/S_H.
!lhl	WRITE(*,*) "Arbitrarily show the absurd value -99.999"
!lhl	WRITE(*,*) "at angles where do not have "// &
!lhl	"a maximum Ri (or radius ra_r)."
!lhl	WRITE(*,*) " "
!lhl	WRITE(*,*) "  \\th_r ^o  ra_r      " &
!lhl          // "  Ri_T        Ri_C        Ri         Ri_d       " &
!lhl          // "  S_M       S_H       S_S      S_S/S_H  "
       endif
!*
...
!981216-30 Write out stability functions, the S's and sisamax.
   16  continue
       if (mytid.eq.0) then
!lhl   	WRITE(*,9150) theta_r_deg,ra_r0,rit0,ric0,ri_r0,rid_r0, &
!lhl                sm_r0,sh_r0,ss_r0,sisamax(itheta_r)
       endif
...
!021210X1 Call version of submodule oursal2 which has an option to use B1={\tau S}^{3/2} (0,0).
	     ...
	     END IF
       if (mytid.eq.0) then
!lhl	     IF(itheta_r.EQ.-n_theta_r_oct) THEN
!lhl	       WRITE(*,*) " "
!lhl	       WRITE(*,*) &
!lhl          "Values at background ra_r=(Ri_T^2 + Ri_C^2)^(1/2)"
!lhl	       WRITE(*,*) "\\th_r ^o   ra_r       " &
!lhl           // "Ri_T       Ri_C       Ri         Ri_d       " &
!lhl           // "(Sl/q)^2   S_M       S_H       S_S       S_S/S_H  "
!lhl	       WRITE(*,*) " "
!lhl	     END IF
       endif
	     sisa1 = ss_r1(itheta_r)/sh_r1(itheta_r)
       if (mytid.eq.0) then
!lhl     	     WRITE(*,9160) theta_r_deg,back_ra_r(itheta_r), &
!lhl                     rit1,ric1,ri_r1,rid_r1,slq2_r1(itheta_r), &
!lhl                  sm_r1(itheta_r),sh_r1(itheta_r),ss_r1(itheta_r), &
!lhl                  sisa1
       endif
...
!030424Z Only write in case zeroshear model enabled.
	...
	CLOSE(68)
!lhl	WRITE(*,*) "************************************************"
!lhl	WRITE(*,*)          "index (N_d^2)/(N^2)  -(\\tau N)^2    "// &
!lhl             "S_M            S_H            S_S            "
!lhl	WRITE(*,*) " "
!lhl!lhl0711	OPEN(UNIT=68,FILE="turb_nd2on2",STATUS="NEW")
!lhl	OPEN(UNIT=68,FILE="turb_nd2on2")
!lhl	WRITE(68,*) "************************************************"
!lhl	WRITE(68,*)          "index (N_d^2)/(N^2)  -(\\tau N)^2    "// &
!lhl             "S_M            S_H            S_S            "
!lhl	WRITE(68,*) " "
!lhl	DO ind2on2 = -mt,mt
!lhl	   WRITE(*,9268) ind2on2,and2on2a1(ind2on2), &
!lhl      amtaun2a1(ind2on2),sma1(ind2on2),sha1(ind2on2),ssa1(ind2on2)
!lhl	   WRITE(68,9268) ind2on2,and2on2a1(ind2on2), &
!lhl      amtaun2a1(ind2on2),sma1(ind2on2),sha1(ind2on2),ssa1(ind2on2)
!lhl	END DO
!lhl	WRITE(*,*) " "
	CLOSE(68)
...
!	  ***SET IT TO THE LAST TABLE ANGLE BEFORE THE CRITICAL ANGLE.****
	    theta_r0 = itheta_r0*deltheta_r
	    theta_r1 = itheta_r1*deltheta_r
	    IF((theta_r0.LE.theta_rcrp).AND.(theta_r.GT.theta_rcrp)) &
       THEN
	      theta_r = theta_r1
	      theta_r0 = theta_r1
	      itheta_r0 = itheta_r1 
...
!        write(*,*) 'i=',ii,'j=',jj
!        write(*,*)
!        write(*,*) 'itheta_r0', itheta_r0
!        write(*,*)
!        write(*,*) 'deltheta_r1', deltheta_r1
!        write(*,*)
!        write(*,*) 'dsm_back_o_dtheta,sm_r1(itheta_r0)',dsm_back_o_dtheta,sm_r1(itheta_r0)
!        write(*,*)
!        write(*,*) 'dsh_back_o_dtheta,sh_r1(itheta_r0)',dsh_back_o_dtheta,sh_r1(itheta_r0)
!        write(*,*)
!        write(*,*)'dss_back_o_dtheta,ss_r1(itheta_r0)', dss_back_o_dtheta,ss_r1(itheta_r0)
!        write(*,*)
!        write(*,*)'dslq2_back_o_dtheta,slq2_r1(itheta_r0)', dslq2_back_o_dtheta,slq2_r1(itheta_r0)
!        write(*,*)
!	endif
...
!*****D
 ...
 9160 FORMAT(F11.3,6E12.4,3F10.6,F9.3)
...
!lhl	  WRITE(*,*) "f_30deg arccosh(N_0/f_0)  =",den
	  icalled=icalled+1
	END IF
...
!
!
!980610 Commented excerpt from the file "sx"
!>
!> sgmt := 0.72;
!         sgmt = .72
!
...
!
!##
!000201 Calculate the timescale ratios in the 'smshsc' routine instead of here.
!C980610 Set \sigma_t0.
!         sgmt = .72D0
!C980610 Calculate {\tau_C \over \tau} and {\tau_{C\theta} \over \tau}.
!	 tcot  = sgmt
!	 tctot = (2.D0/15.D0)*sgmt
!C980615 "tpt/tau" and "tpc/tau" from the "sx" excerpt
!         tptot = 1.D0/(5.D0*(1+1/sgmt))
!         tpcot = 1.D0/(5.D0*(1+1/sgmt))
!C
!000127-0201 Timescale ratios are now calculated in the 'smshsc' subroutine.
...
#else
	  tabindrid = SIGN(FLOAT(m0) + ((LOG(ABS(rid)) - LOG(rid_1(m0)))/LOG(rat)), rid)
#endif
#if (defined D_PRECISION)
...
#else
	  tabindri = SIGN( FLOAT(m0) + ((LOG(ABS(ri)) - LOG(ri_1(m0)))/LOG(rat)), ri)
#endif
#if (defined D_PRECISION)
